...
PYTHONPATH=. "$PYTHON_BIN" -m unittest \
  ...
  tests.test_owner_truth_candidate_decision_authorization_evidence_migration_contract \
  tests.test_release_policy \
  tests.test_route_ownership_registry
PYTHONPATH=. "$PYTHON_BIN" -m py_compile \
  app/domain/owner_truth/candidate_decisions.py \
...
assert "ownerTruthCandidateReview" in ReleasePolicyService._CLOSED_PILOT_OPT_IN_FEATURES
assert not Settings().release_policy_closed_pilot_features
print("Owner Truth closed-pilot Candidate review gate passed")
PY
