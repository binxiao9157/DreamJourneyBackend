...
这份围栏最初不实现 `cooldownUntil`、ThreadPreference 或自动恢复。当前终版需求已补充
一个严格例外：服务端确认已到期的 `paused + cooldown` Thread 可以进入“接着聊”候选池，
但不会写回 Session 或绕过 `doNotAsk`。当前规则以
`docs/backend/2026-07-23-owner-truth-thread-preferences.md` 为准。
...
未到期 `cooldown`、`doNotAsk`、`skipOnce`、暂停/结束 Session、缺失 Session 或多 Session
关联均 fail closed。唯一例外是服务端从 ThreadPreference 读取到已到期的 `cooldown`，且
该 Session 仍严格为 `paused + cooldown`；该例外只产生 value-free 的连续性候选，不会恢复
Session 或改变既有生命周期。
...
3. 服务端只从当前 Owner-confirmed `DimensionProjection` 与唯一 `active + open` 的
   InterviewSession/ConversationThread 生成候选；另可从一条服务端验证已到期的
   `paused + cooldown` Thread 生成连续性候选；不写入 Candidate、Memory、Conversation 或
   Provider 状态；
4. 默认生成一条 `breadth` 候选，表示已经确认但未完整覆盖的维度缺口；显式 Owner
   `SavedContinuationCue` 或已到期 `cooldown` 可以额外生成一条 `continuity`。后者使用
   `continueElapsedCooldown` / `elapsedCooldownContinuation`，不会从覆盖量推断原始话题。
   SavedContinuationCue 的合同与失效条件见
   `docs/backend/2026-07-23-owner-truth-saved-continuation-cue.md`；
...
- `POST /v2/vaults/{vaultId}/knowledge-recommendations/plan` 同样默认关闭；它现可证明
  服务端 breadth、显式 SavedContinuationCue 和已到期 cooldown 的 value-free 规划合同，
  不代表公开知识地图已完成；
- 返回值不增加消息、Thread metadata 或推荐问题正文；
- 此变更只补 Phase 4 M0-B 的 G0 授权前置条件和已到期 cooldown 的读取语义，不代表公开双推荐、
  知识地图、自动 Thread 合并、G1 UIQA、G2 长期持久化验收或 G4 公开发布已经完成。
