...
- `OWNER_TRUTH_CONTEXT_AUTHORITY_ENABLED=false` 控制 `/context/build` 是否使用 V4 `MemoryProjection`。生产开启后，所有已登录且访问本人 Vault 的 Owner 都走该链路，测试账号白名单不参与判断；它仍不接受客户端伪造 cohort、家庭角色或任意 `digitalHumanId` 作为进入条件。投影缺失或重建中时返回空 V4 Context，绝不回读旧 Archive/KBLite。旧变量 `OWNER_TRUTH_CONTEXT_AUTHORITY_CLOSED_PILOT_ENABLED` 仅作为部署迁移兼容别名。
