...
- `RELEASE_POLICY_AUTHENTICATED_OWNER_V4_ENABLED=true` 将 Source、Candidate、MemoryVersion、Projection/Search、Owner Echo、人生地图、访谈结果、家庭贡献、人格和已就绪的 Voice/Digital Human Provider 能力授权给所有已登录 Owner；登录测试账号白名单只负责认证，不参与产品权限。媒体、Voice 和 Digital Human 仍需各自的服务端 Provider readiness，未就绪时返回 `capabilityUnavailable`，不会伪装成可用。
- `RELEASE_POLICY_CLOSED_PILOT_OWNER_IDS` 是服务端维护的 closed-pilot 用户 ID 白名单；客户端传入的 cohort header 或 query 不会授予资格。`RELEASE_POLICY_CLOSED_PILOT_FEATURES` 仅接受 `ownerTextCaptureV1`、`ownerTruthCandidateReview`、`echoGuidedRecommendations`、`ownerTruthLifeMap`、`ownerTruthFamilyContribution`：前两项分别开启 M0 文字 Source 写入和候选确认，之后两项分别开启“接着聊/换个角度”和只读人生地图；最后一项只允许 Owner 对一位已接受的家庭成员签发“提交静态资料”授权。家庭成员没有 Vault 读取、候选确认、人格、声音、数字人、发布或访客权限；关闭该服务端 feature 或移除 Owner 白名单会立即停止旧授权继续提交。它们都只在该白名单内显式开启，默认关闭；其他功能名会使服务启动失败。文字 Source 写入始终要求当前用户会话、服务端策略 capture 和匹配的 Vault authority epoch；网络或策略失败时客户端只能保留本地草稿，不能显示“云端已封存”。
- 生产部署模板显式设置 `OWNER_TRUTH_CONTEXT_AUTHORITY_ENABLED=true`，让 `/context/build` 使用 V4 `MemoryProjection`。所有已登录且访问本人 Vault 的 Owner 都走该链路，测试账号白名单不参与判断；它仍不接受客户端伪造 cohort、家庭角色或任意 `digitalHumanId` 作为进入条件。投影缺失或重建中时返回空 V4 Context，绝不回读旧 Archive/KBLite。代码级安全默认值仍为 `false`，生产只有事故回滚时才可临时关闭；旧变量 `OWNER_TRUTH_CONTEXT_AUTHORITY_CLOSED_PILOT_ENABLED` 仅作为部署迁移兼容别名。
...
- `OWNER_TRUTH_DO_NOT_ASK_REACTIVATION_PREFLIGHT_ENABLED=false` 是独立的自然重开预检开关。开启后，已暂停的 `doNotAsk` Session 收到明确的“愿意重新聊”表达时，只返回无正文的 `restoreDoNotAsk` 确认要求；不会写入该条输入、不会自动恢复边界，也不会创建 Thread/Candidate/MemoryVersion 或公开 UI。真正恢复仍必须走既有 `confirmed=true` 的 Owner 命令。
...
- `OWNER_TRUTH_INTERVIEW_SESSION_OUTCOME_READ_QA_ENABLED=false` 是独立的 Phase 4C 会话结果读取开关。它要求 Candidate QA、维度确认 QA、认证 Owner 会话和 `X-DreamJourney-QA-Owner-Truth: 1`；只返回本会话已确认的 `MemoryVersion` 数量、审核批次计数和当前可续状态，不返回对话、Candidate、记忆正文或原始 Provider 数据，也不写入公开 Echo。
- `OWNER_TRUTH_LIFE_MAP_READ_QA_ENABLED=false` 是独立的 Phase 4C 人生地图读取开关。它要求 Candidate QA、维度确认 QA、认证 Owner 会话和 `X-DreamJourney-QA-Owner-Truth: 1`；只返回六个稳定维度的覆盖/缺口计数、当前 Thread 状态和可逆关联，不返回记忆正文、转录、Candidate、Source、模型主题或 Provider 数据，也不写入公开 Echo。
- `OWNER_TRUTH_MEMORY_SEARCH_READ_QA_ENABLED=false` 是独立的 Phase 4C SearchDocument 读取开关。它要求 Candidate QA、认证 Owner 会话和 `X-DreamJourney-QA-Owner-Truth: 1`；只从当前 confirmed `MemoryVersion` Projection 生成请求内检索材料，当前仅提供明确标识的 `deterministicTextFallback`，返回引用型命中而非正文、Source、Candidate 或查询文本。它不等同于公开语义搜索，也不接入 KBLite 或 Echo。
- `ASYNC_EFFECT_V1_ENABLED=false`、`ASYNC_EFFECT_WORKER_ENABLED=false`、`OWNER_TRUTH_CANDIDATE_EXTRACTION_WORKER_ENABLED=false`、`OWNER_TRUTH_MEMORY_PROJECTION_WORKER_ENABLED=false` 和 `BUSINESS_MESSAGE_PROJECTION_WORKER_ENABLED=false` 默认关闭 V4 异步副作用执行。`0013_async_effects_kernel` 只部署可重建的协调 schema；API 不得把本地 timer、notification 或 schema 存在解释为服务端完成。现有 `async-effect-worker` 与 `async-effect-scheduler` Compose profile 仍只允许值无关的 shadow observation。Owner Truth 的 Candidate/Projection worker 只有在全部开关开启后，再显式启动 `owner-truth-worker` Compose profile 才会持续 claim 对应 typed job；`business-message-worker` profile 只写入内部 metadata shadow，不写公开 mailbox 或调用通知 Provider。它们都不改变 `/context/build`、KBLite legacy writer 或公开 Echo。
