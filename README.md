...
- `OWNER_TRUTH_CONTEXT_AUTHORITY_CLOSED_PILOT_ENABLED=false` 默认保持 `/context/build` 的旧链路。只有它与上面的服务端白名单、`ownerTruthCandidateReview` feature 以及 Owner Truth worker profile 同时就绪时，白名单内“本人个人回响”才会从当前已确认的 V4 `MemoryProjection` 构建 Context。它不会接受 QA header、客户端 cohort、家庭角色或任意 `digitalHumanId` 作为进入条件；投影缺失或重建中时返回空 V4 Context，绝不回读旧 Archive/KBLite。
