...
class InMemoryVoiceDHAuthorityRepository:
    ...
    def __init__(self) -> None:
        ...
        self._receipts: dict[tuple[str, str], VoiceDHAuthorityReceipt] = {}
    ...
    def record_blocked_profile_version(
        ...
        *,
        ...
        receipt_key = (record.context.vault_id, record.command.command_id_hash)
        with self._lock:
    ...
    def snapshot(self) -> dict[str, int]:
        with self._lock:
            return {"profileVersionCount": len(self._profiles), "receiptCount": len(self._receipts)}
...
class PostgresVoiceDHAuthorityRepository:
    ...
    def _normalized_row(row: Any, expected: dict[str, object]) -> dict[str, object] | None:
        if row is None:
            return None
        return {
            key: str(row[key]) if key in {"id", "resource_id"} and row[key] is not None else row[key]
            for key in expected
        }
