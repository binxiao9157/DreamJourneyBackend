-- migration:owner_truth_correction_resolution_currentness
--
-- Tighten the already-default-off Owner Truth correction resolver.  A pending
-- request must not become terminal after either the cited Source or the
-- private correction Source has been redacted/deleted, and a stale predecessor
-- may not be recorded as a rejected correction by a direct database writer.
-- This only replaces the validation function used by the existing trigger.
