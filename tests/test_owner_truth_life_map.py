...
class OwnerTruthLifeMapTests(unittest.TestCase):
    def setUp(self) -> None:
        self.owner_id = "owner-life-map"
        self.vault_id = "vault-life-map"
        ...
        self.memory = OwnerTruthMemoryProjectionInput(
            ...
            content_hash=_hash({"claim": "private life-map content must not leak"}),
            content={"claim": "private life-map content must not leak"},
    ...
    def _thread(self, *, paused_cooldown: bool = False) -> OwnerTruthConversationThreadAuthoritySnapshot:
        ...
        if str(reader.snapshot.get("state") or "") == "ready":
            OwnerTruthKnowledgeDimensionConfirmationService(confirmation_store, enabled=True).confirm(
                ...
                command=OwnerTruthKnowledgeDimensionConfirmationCommand(
                    command_id="life-map-confirmation",
    ...
    def test_projects_confirmed_dimensions_and_reversible_thread_associations(self) -> None:
        active = self._thread()
        cooldown = self._thread(paused_cooldown=True)
        result = OwnerTruthLifeMapReadService(
            self._store(threads=(active, cooldown), cues=(self._cue(active), self._cue(cooldown)))
        ).read(context=self.context)

        self.assertEqual(result.state, OwnerTruthKnowledgeDimensionReadState.READY)
        assert result.projection is not None
        self.assertEqual(len(result.projection.dimensions), 6)
        decision = next(
            item
            for item in result.projection.dimensions
            if item.dimension is KnowledgeDimension.KEY_DECISIONS
        )
        self.assertEqual(decision.evidence_count, 1)
        self.assertEqual(decision.covered_facet_count, 1)
        self.assertEqual(decision.anchored_thread_count, 2)
        self.assertEqual(len(result.projection.associations), 1)
        rendered = str(result.value_free_summary())
        self.assertNotIn("private life-map", rendered)
        self.assertNotIn("memoryVersionId", rendered)
        self.assertNotIn("sourceId", rendered)

    def test_filters_stale_cues_without_retaining_a_stale_map_edge(self) -> None:
        active = self._thread()
        cooldown = self._thread(paused_cooldown=True)
        result = OwnerTruthLifeMapReadService(
            self._store(
        ...
        self.assertEqual(result.projection.unmapped_thread_count, 1)

    def test_rebuilding_projection_returns_no_retained_life_map(self) -> None:
        result = OwnerTruthLifeMapReadService(
            self._store(
