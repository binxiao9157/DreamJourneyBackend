...
class OwnerTruthThreadSummaryReadAPITests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _login(phone: str) -> tuple[str, dict[str, str]]:
        response = client.post(
            "/auth/login",
            json={"phone": phone, "nickname": "Thread Summary QA", "password": "password123"},
        )
    ...
    def _activate_memory(self, *, vault_id: str, owner_id: str) -> tuple[str, str]:
        source_id = str(uuid4())
        content = {"claim": "private thread summary API content must not leak"}
        ...
        OwnerTruthCandidateReviewService(self.store).decide_and_activate(
            command=OwnerTruthCandidateReviewCommand(
                command_id="thread-summary-api-memory-accept-001",
    ...
    def test_contract_is_hidden_when_its_separate_flag_is_disabled(self) -> None:
        ...
        response = client.post(
            "/v2/vaults/vault-thread-summary-hidden/thread-summaries/read",
            headers=headers,
            json={},
        )
    ...
    def test_owner_reads_ready_value_free_summary_without_memory_text(self) -> None:
        owner_id, headers = self._login("13800139422")
        vault_id = "vault-thread-summary-api"
        ...
        confirmation = client.post(
            ...
            headers=headers,
            json={
                "commandId": "thread-summary-api-confirm-001",
        ...
        body = response.json()
        self.assertEqual(body["schemaVersion"], "owner-truth-thread-summary-read-response-v1")
        self.assertEqual(body["vaultId"], vault_id)
        summary = body["threadSummaries"]
        self.assertEqual(summary["state"], "ready")
        projection = summary["projection"]
        self.assertEqual(projection["threadCount"], 0)
        self.assertEqual(projection["associationCount"], 0)
        self.assertNotIn("private thread summary API content", response.text)
        self.assertNotIn("claim", response.text)

    def test_payload_with_unsupported_fields_is_rejected(self) -> None:
        _owner_id, headers = self._login("13800139423")
        response = client.post(
            "/v2/vaults/vault-thread-summary-invalid/thread-summaries/read",
            headers=headers,
        ...
        self.assertEqual(
            response.json()["detail"]["code"],
            "ownerTruthThreadSummaryReadInvalid",
        )
