...
class ReleasePolicyServiceTests(unittest.TestCase):
    ...
    def test_owner_media_capture_requires_an_independent_closed_pilot_feature_grant(self):
        disabled = ReleasePolicyService().build_snapshot(
            audience="owner",
            cohort="closedPilotAdultSelf",
            client_build=1,
...
class ReleasePolicyCommandGateTests(unittest.TestCase):
    ...
    def test_route_inventory_covers_hidden_commands_and_dynamic_archive_payloads(self):
        ...
        self.assertEqual(
            gate.feature_for_request(
                "POST",
                "/v2/vaults/vault-a/candidates/candidate-a/changeset-preview",
                {},
            ),
            "ownerTruthCandidateReview",
        )
        self.assertEqual(
            gate.feature_for_request(
                "POST",
                "/v2/vaults/vault-a/candidates/candidate-a/decisions",
                {},
            ),
            "ownerTruthCandidateReview",
        )
        ...
        self.assertEqual(
            gate.feature_for_request(
                "POST",
                "/v2/vaults/vault-a/guided-recommendations/activate",
                {},
            ),
            "echoGuidedRecommendations",
        )
        self.assertEqual(
            ...
            "POST /v2/vaults/*/guided-recommendations/activate",
        )
        self.assertEqual(
            gate.route_label_for_request(
                "POST",
