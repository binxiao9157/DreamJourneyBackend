...
class AsyncEffectsPostgresSmokeContractTests(unittest.TestCase):
    def test_smoke_is_disposable_and_covers_required_g2_boundaries(self):
        ...
        self.assertIn("same Source extraction must persist one immutable result and replay", source)
        self.assertIn("synthetic extraction must persist one pending Candidate in Postgres", source)
        self.assertIn("candidate extraction must retain one Consumer Inbox completion receipt", source)
