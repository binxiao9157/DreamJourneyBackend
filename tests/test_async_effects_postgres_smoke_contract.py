...
class AsyncEffectsPostgresSmokeContractTests(unittest.TestCase):
    def test_smoke_is_disposable_and_covers_required_g2_boundaries(self):
        ...
        self.assertIn("database_created = False", source)
        self.assertIn("if database_created:", source)
        self.assertIn("expected_migration_head = load_migrations(migrations_dir)[-1].version", source)
        self.assertIn("all current migrations must apply before the async-effect smoke", source)
        ...
        self.assertIn("default-disabled Source worker must terminalize one live extraction job", source)
        self.assertIn("Source worker must persist one pending Candidate in Postgres", source)
        self.assertIn("worker result must not echo private Source text", source)
        ...
        self.assertIn("stale target admission must write a typed blocked completion in the same UoW", source)
        self.assertIn("typed blocked completion must retain one immutable business receipt", source)
