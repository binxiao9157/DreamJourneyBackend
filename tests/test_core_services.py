...
class AccountDeletionAPITests(unittest.TestCase):
    ...
    def test_account_delete_soft_deletes_and_login_restores_once_by_phone(self):
        ...
        self.assertTrue(deletion["dataExportSupported"])
        self.assertEqual(deletion["dataExportState"], "availableBeforeDeletionOnly")
...
class VoiceCloneProfileAPITests(HiddenStageContractTestCase):
    ...
    def test_voice_clone_profile_contract_requires_authorization_and_persists_lifecycle(self):
        ...
        self.assertIn("Provider 停用或删除回执", profile["disableContract"])
        self.assertIn("尚未产生样本、训练产物或 Provider 资产的删除回执", profile["deleteContract"])
        self.assertIn("不能宣称第三方清理已完成", profile["deleteContract"])
        self.assertEqual(profile["exitState"], "active")
        self.assertFalse(profile["accessRevoked"])
        self.assertEqual(profile["localCleanupState"], "notRequested")
        self.assertEqual(profile["providerCleanupState"], "notRequested")
        self.assertFalse(profile["providerCleanupReceiptAvailable"])
        self.assertNotIn("rawSampleURL", profile)
        self.assertNotIn("sampleLocalPath", profile)
        self.assertNotIn("audioBase64", profile)
        ...
        self.assertEqual(disabled_profile["exitState"], "accessRevoked")
        self.assertTrue(disabled_profile["accessRevoked"])
        self.assertEqual(disabled_profile["localCleanupState"], "retained")
        self.assertEqual(disabled_profile["providerCleanupState"], "notRequested")
        self.assertFalse(disabled_profile["providerCleanupReceiptAvailable"])
