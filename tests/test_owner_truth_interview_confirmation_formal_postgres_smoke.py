...
class OwnerTruthInterviewConfirmationFormalPostgresSmokeTests(unittest.TestCase):
    def test_runner_requires_an_explicit_database_and_keeps_formal_route_separate_from_qa(self) -> None:
        ...
        self.assertIn('FORMAL_SMOKE_BATCH_CANDIDATE_SUMMARY', smoke)
        self.assertIn('"selectionMode": "projectionCitationOrder"', smoke)
        self.assertIn('concurrent_client = TestClient', smoke)
        self.assertIn('decision_status AS decision', smoke)
